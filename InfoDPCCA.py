...
def recon_fig(model, test_loader, dim_idx, data_idx=0, fin=False):
    ...
    plt.plot(x1, label='Original x1[0,:,0]', linestyle='-', marker='o')
    plt.plot(x1_recon_loc, label='Reconstructed x1[0,:,0]', linestyle='--', marker='x')
...
def recon_fig_z0(model, test_loader, dim_idx, data_idx=0):
    # by default, we select first test sample of x_1
    model.eval()
    ...
    plt.axvline(x=235, color='red', linestyle='--')
