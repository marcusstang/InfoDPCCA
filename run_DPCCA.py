...
def main():
    checkpoint_dir = "DPCCA_checkpoints"
    # load AAL data
    ...
    M2 = D2PCCA(
        x_dims=[116, 116],
        emission_dims=[200, 200],
        z_dims=[5, 20, 20],
        transition_dims=[20, 30, 30],
        rnn_dim=600,
        rnn_layers=1,
    ).to(device)
    ...
    start_epoch = load_model(M2, adam2, checkpoint_dir=checkpoint_dir)
    ...
    if start_epoch < NUM_EPOCHS:
        for epoch in range(start_epoch, NUM_EPOCHS+1):
            total_epoch_loss_train = train_D2PCCA(svi2, train_loader, true_latent=True)
            ...
            if epoch > 0 and epoch % save_every == 0:
                save_model(M2, adam2, epoch)

    #recon_fig_D2PCCA(M2, train_loader, 5)
    #print(recon_loss_D2PCCA(M2, train_loader, True))
    ...
    _, _, recon_z, _ = M2([x1, x2])
    recon_z0 = recon_z[:, :-1, :M2.z_shared_dim]
    ...
    if new_K:
        ...
        np.save("K_D2_AAL.npy", K)
    else:
        K = np.load("K_D2_AAL.npy")
    ...
    plt.title("DPCCA: TCK matrix (sorted)")
    plt.colorbar(h)
    ...
    fig.write_html('scatter_3d_D2.html')
